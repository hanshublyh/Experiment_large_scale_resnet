...
class Env:
    ...
    def step_1a(self, act):
        ...
        # 当前执行哪个服务
        # 然后迁移并更新
        state, did, inc, dec = self.get_new_state(act)
        reward = self.give_reward(act, did, inc, dec, 0)
    ...
    def step(self, act, round):
        ...
        # 然后迁移并更新
        state, did, inc, dec = self.get_new_state(act)
        reward = self.give_reward(act, did, inc, dec, round)
    ...
    def give_reward(self, act, did, inc, dec, round):
        reward = 0
        '''
        stop = 0
